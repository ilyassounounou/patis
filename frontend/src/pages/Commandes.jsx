  ...
  try {
    ...
    setCommandes(prevCommandes => 
      prevCommandes.map(cmd => 
        cmd._id === commandeId ? data.commande : cmd
      )
    );
...
const deleteCommande = async (commandeId) => {
  if (!window.confirm('هل أنت متأكد من أنك تريد حذف هذا الطلب؟')) {
    return;
  }
  
  try {
    setLoading(true);
    await axios.delete(`${API_URL}/${commandeId}`);
    
    // Remove the commande from the list
    setCommandes(prevCommandes => 
      prevCommandes.filter(cmd => cmd._id !== commandeId)
    );
    
    // If the deleted commande is currently displayed, clear it
    if (commande && commande._id === commandeId) {
      setCommande(null);
    }
            ...
            <form onSubmit={submitCommande}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 border-t pt-4">
                ...
                <div className="flex flex-col justify-end">
                  ...
                  <span className={`text-lg font-bold ${calculateReste() > 0 ? 'text-orange-600' : 'text-green-600'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                    ...
                    commande.status === 'preparing' ? 'bg-yellow-100 text-yellow-800' :
                    commande.status === 'ready' ? 'bg-blue-100 text-blue-800' :
                  ...
                  <tbody>
                    {commande.items.map((item, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="py-3 text-right">DH {(item.price * item.quantity).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-6 border-t border-b">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-blue-700">DH {commande.total.toFixed(2)}</p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-green-700">DH {commande.avance.toFixed(2)}</p>
                ...
                <div className="bg-orange-50 p-4 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-orange-700">DH {commande.reste.toFixed(2)}</p>
                </div>
              </div>
            </div>
...
{/* All Commandes List */}
<div className="mt-10">
  <h2 className="text-2xl font-bold mb-6 text-blue-800 border-b-2 border-blue-200 pb-2">كل الطلبات</h2>
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <span className={`text-xs font-mono px-2 py-1 rounded-md ${
                  cmd.reste === 0 ? 'bg-green-100 text-green-800' : 'bg-white text-blue-800'
                }`}>
                  {cmd.code}
                </span>
              </div>
          ...
          <div className="p-4">
            ...
            <div className="mb-4">
              <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                ...
                cmd.status === 'preparing' ? 'bg-yellow-100 text-yellow-800' :
                cmd.status === 'ready' ? 'bg-blue-100 text-blue-800' :
                cmd.status === 'cancelled' ? 'bg-red-100 text-red-800' :
                'bg-gray-100 text-gray-800'
              }`}>
            ...
            {/* Items List */}
            <div className="mb-4">
              <h4 className="text-xs font-semibold text-gray-700 mb-2 border-b pb-1">المنتجات</h4>
              {Array.isArray(cmd.items) && cmd.items.length > 0 ? (
                <div className="space-y-1">
                  {cmd.items.slice(0, 3).map((item, idx) => (
                    <div key={idx} className="flex justify-between text-xs">
                      <span className="truncate max-w-[60%]">{item.name}</span>
                      <span>{item.quantity} × {item.price} د.م</span>
                    </div>
                  ))}
                  {cmd.items.length > 3 && (
            ...
            <div className={`p-3 rounded-lg ${
              ...
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-sm font-medium text-green-600">{cmd.avance?.toFixed(2)} د.م</span>
              </div>
              <div className="flex justify-between items-center">
            ...
            <div className="flex justify-between mt-3 space-x-2">
              ...
              <button
                onClick={() => deleteCommande(cmd._id)}
